...
Partial Class complementospago
    Inherits System.Web.UI.Page
    Public ssql As String
    Public objdatos As New Cls_Funciones
    Private Sub complementospago_Load(sender As Object, e As EventArgs) Handles Me.Load
        fnLlenarcolumnas()
        fnLlenarListado()
        ...
        If Request.QueryString.Count > 0 Then
            If Request.QueryString("action") = "v" Then 'Ver
                fnVer()
            End If
            If Request.QueryString("complemento") <> "" Then

                If Request.QueryString("XML") <> "" Then
                    fnDescargarXML(Request.QueryString("complemento"))
                End If
                If Request.QueryString("PDF") <> "" Then
                    If objdatos.fnObtenerCliente.ToUpper.Contains("AIO") Or objdatos.fnObtenerCliente.ToUpper.Contains("PMK") Then
                        ...
                        fnDescargarPDF(Request.QueryString("complemento"))
                    Else
                        objdatos.fnLog("exportar pdf", "Descargar PDF v2:" & objdatos.fnObtenerCliente.ToUpper.Contains("PMK"))
                        fnDescargarPDFv2(Request.QueryString("complemento"))
                    End If
    ...
    Public Sub fnDescargarXML(docEntry As Int64)
        ...
        If objdatos.fnObtenerCliente.ToUpper.Contains("AIO") Then
            ...
            ssql = "SELECT  Convert(varchar(7),docdate,120)+'\' + cardcode + '\PR\'  as Ruta1 , reportId + '.xml' as Ruta,T0.Docdate as Fecha FROM ORCT T0 WITH(nolock) INNER JOIN ECM2 T1  WITH(nolock) ON T1.SrcObjType ='24' AND T1.SrcObjAbs =T0.docEntry WHERE T0.docentry=" & "'" & docEntry & "'"
            dtXML = objdatos.fnEjecutarConsultaSAP(ssql)
            If dtXML.Rows.Count > 0 Then
    ...
    Public Sub fnDescargarPDF(Docentry As Int64)
        ...
        reporte.Load(Server.MapPath("~") & "CP.rpt")
        ...
        Try
            ...
            ssql = "SELECT docnum from ORCT where docentry=" & "'" & Docentry & "'"
            ...
            reporte.ExportToHttpResponse(ExportFormatType.PortableDocFormat, Response, True, "CM-" & docnum)
            reporte.Dispose()
    ...
    Public Sub fnLlenarListado()
        ...
        ssql = objdatos.fnObtenerQuery("ComplementosB2B")
        ...
        For i = 0 To dtDocumentos.Rows.Count - 1 Step 1
            sHtml = sHtml & "<tr>"
            sHtml = sHtml & "<td class='mas'><i class='fa fa-chevron-right preview-popup' aria-hidden='true' href='factura-modal.aspx?Doc=" & dtDocumentos.Rows(i)(0) & "&QueryDoc=ComplementosB2Bdetalle'></i></td>"
