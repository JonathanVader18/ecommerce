...
Partial Class notascredito
    ...
    Private Sub notascredito_Load(sender As Object, e As EventArgs) Handles Me.Load
        fnLlenarcolumnas()
        fnLlenarListado()
        ...
        If Request.QueryString.Count > 0 Then
            ...
            If Request.QueryString("nc") <> "" Then
                ...
                If Request.QueryString("XML") <> "" Then
                    fnDescargarXML(Request.QueryString("nc"))
                End If
                If Request.QueryString("PDF") <> "" Then
                    If objdatos.fnObtenerCliente.ToUpper.Contains("AIO") Or objdatos.fnObtenerCliente.ToUpper.Contains("PMK") Then
                        ...
                        fnDescargarPDF(Request.QueryString("nc"))
                    Else
                        objdatos.fnLog("exportar pdf", "Descargar PDF v2:" & objdatos.fnObtenerCliente.ToUpper.Contains("PMK"))
                        fnDescargarPDFv2(Request.QueryString("nc"))
                    End If
    ...
    Public Sub fnDescargarXML(docEntry As Int64)
        ...
        If dtXML.Rows.Count > 0 Then
            ...
            File.Copy(rutaXML & "\" & CStr(dtXML.Rows(0)("ruta")).Replace(".jpg", ".xml"), Server.MapPath("~") & "\FacturasTEMP\" & CStr(dtXML.Rows(0)("ruta")).Replace(".jpg", ".xml"), True)

        End If

        If objdatos.fnObtenerCliente.ToUpper.Contains("AIO") Then
            rutaXML = "\\AUTIMP-sap\Archivos de SAP\XML\AIO090330JP2\"
            ssql = "SELECT  Convert(varchar(7),docdate,120)+'\' + cardcode + '\RC\'  as Ruta1 , reportId + '.xml' as Ruta,T0.Docdate as Fecha FROM ORIN T0 WITH(nolock) INNER JOIN ECM2 T1  WITH(nolock) ON T1.SrcObjType ='14' AND T1.SrcObjAbs =T0.docEntry WHERE T0.docentry=" & "'" & docEntry & "'"
            dtXML = objdatos.fnEjecutarConsultaSAP(ssql)
            If dtXML.Rows.Count > 0 Then
                If CDate(dtXML.Rows(0)("Fecha")) > "2023-03-31" Then
                    rutaXML = "\\AUTIMP-sap\export\autimp-sap.autimpocc.com.mx\SBO_AIO\CFDi\AIO090330JP2\"
                End If

                If File.Exists(rutaXML & "\" & CStr(dtXML.Rows(0)("ruta1")) & "\" & CStr(dtXML.Rows(0)("ruta")).Replace(".jpg", ".xml")) Then
                    File.Copy(rutaXML & "\" & CStr(dtXML.Rows(0)("ruta1")) & "\" & CStr(dtXML.Rows(0)("ruta")).Replace(".jpg", ".xml"), Server.MapPath("~") & "\FacturasTEMP\" & CStr(dtXML.Rows(0)("ruta")).Replace(".jpg", ".xml"), True)
                Else
                    objdatos.fnLog("NC", "No existe:" & rutaXML & "\" & CStr(dtXML.Rows(0)("ruta1")) & "\" & CStr(dtXML.Rows(0)("ruta")).Replace(".jpg", ".xml"))
                End If
                '            File.Copy(rutaXML & "\" & dtXML.Rows(0)("Fecha") & "\" & dtXML.Rows(0)("CardCode") & "\IN\" & dtXML.Rows(0)("eDocNum") & ".xml", Server.MapPath("~") & "\FacturasTEMP\" & dtXML.Rows(0)("eDocNum") & ".xml")
        ...
        Try
            ...
            Context.Response.AddHeader("content-disposition", "attachment;filename=" & CStr(dtXML.Rows(0)("ruta")).Replace(".jpg", ".xml"))
        ...
        Catch ex As Exception
            objdatos.fnLog("NC ex", ex.Message)
        End Try
    ...
    Public Sub fnDescargarPDFv2(docEntry As Int64)
        ...
        If dtXML.Rows.Count > 0 Then
            ...
            '            File.Copy(rutaXML & "\" & dtXML.Rows(0)("Fecha") & "\" & dtXML.Rows(0)("CardCode") & "\IN\" & dtXML.Rows(0)("eDocNum") & ".xml", Server.MapPath("~") & "\FacturasTEMP\" & dtXML.Rows(0)("eDocNum") & ".xml")
            File.Copy(rutaXML & "\" & CStr(dtXML.Rows(0)("ruta")).Replace(".jpg", ".pdf"), Server.MapPath("~") & "\FacturasTEMP\" & CStr(dtXML.Rows(0)("ruta")).Replace(".jpg", ".pdf"), True)
        ...
        Try
            ...
            Response.ClearContent()
            Context.Response.Clear()
            Context.Response.AddHeader("content-disposition", "attachment;filename=" & CStr(dtXML.Rows(0)("ruta")).Replace(".jpg", ".pdf"))
            Response.ContentType = "text/xml"
            Response.WriteFile(Server.MapPath("~") & "\FacturasTEMP\" & CStr(dtXML.Rows(0)("ruta")).Replace(".jpg", ".pdf"))
            Response.End()
    ...
    Public Sub fnDescargarPDF(Docentry As Int64)
        ...
        reporte.Load(Server.MapPath("~") & "NC.rpt")
        ...
        Try
            ...
            ssql = "SELECT docnum from ORIN where docentry=" & "'" & Docentry & "'"
            ...
            reporte.ExportToHttpResponse(ExportFormatType.PortableDocFormat, Response, True, "NC-" & docnum)
            reporte.Dispose()
    ...
    Public Sub fnLlenarListado()
        ...
        ssql = objdatos.fnObtenerQuery("NCB2B")
        ...
        For i = 0 To dtDocumentos.Rows.Count - 1 Step 1
            sHtml = sHtml & "<tr>"
            sHtml = sHtml & "<td class='mas'><i class='fa fa-chevron-right preview-popup' aria-hidden='true' href='factura-modal.aspx?Doc=" & dtDocumentos.Rows(i)(0) & "&QueryDoc=NcB2Bdetalle'></i></td>"
